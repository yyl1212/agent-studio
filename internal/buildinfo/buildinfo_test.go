...
func TestPublicVersionsForV050(t *testing.T) {
	if agentnode.Version != "0.5.0" {
		t.Fatalf("sdk=%s", agentnode.Version)
	}
	info := resolve("", nil, false)
	if info.Version != "0.5.0-dev" || info.SDKVersion != "0.5.0" || info.APIVersion != "agent-studio.dev/v1alpha1" {
		t.Fatalf("%+v", info)
	}
}
...
func TestResolveBuildInfo(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			ok: true,
			want: Info{
				Version: "v0.2.0-rc.9", SDKVersion: "0.5.0",
		...
		},
		{
			...
			ok: true,
			want: Info{
				Version: "v0.2.0-rc.1", SDKVersion: "0.5.0",
				APIVersion: "agent-studio.dev/v1alpha1",
		...
		},
		{
			...
			ok: true,
			want: Info{
				Version: "0.5.0-dev", SDKVersion: "0.5.0",
				APIVersion: "agent-studio.dev/v1alpha1",
