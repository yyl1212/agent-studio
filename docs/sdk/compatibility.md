...
本文冻结 Agent Studio Go SDK v0.2 的兼容边界。

## 协议与 SDK 版本

- v0.2 使用 API Version `agent-studio.dev/v1alpha1`。
...
Capability 在 v0.2 只用于展示、审计和未来调度，不提供进程、网络、密钥或文件系统隔离。节点仍与 API 运行在同一进程中；宿主部署者必须只加载可信代码，并使用容器、操作系统权限和网络策略建立真正的隔离边界。
