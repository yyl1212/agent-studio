...
- 规范化模板文件最大 2 MiB；preview/import 的完整 HTTP JSON envelope 同样最大 2 MiB，复用现有 `maxJSONBodyBytes`。
- 最多 500 个节点、2000 条边。
- 节点 config JSON 最大嵌套深度 64。
- 前端先检查原始文件，再检查 preview/import 的序列化 envelope；服务端 2 MiB 限制仍是最终权威。
...
1. 选择 `.json` 文件；只允许单文件，客户端先检查原始文件不超过 2 MiB，并在 preview/import 前检查序列化后的完整请求 envelope 仍不超过 2 MiB。
...
| `docs/workflow-templates.md` | 中文模板导入导出、格式与安全边界说明 |
...
| `apps/web/src/app/styles.css`、`apps/web/src/features/studio/studio.css` | 最小导入预览和工具栏状态样式 |
| `README.md` | 增加工作流模板使用入口 |
