...
- 最多 512 个节点声明。
- 字符串按 Unicode code point 设置固定上限：Module/import path 512、displayName 128、description 2048、license 128、repository 2048、Node API 128、节点 type 256、所有版本字段 128；Go 校验与 JSON Schema 使用相同数值。
...
v1alpha2 在 `spec` 中增加可选依赖提示。`nodePackages` 字段本身可省略；缺失时按空数组迁移，规范化导出始终显式写出 `nodePackages: []`，因此“可选”只影响输入兼容，不产生 `null`：
...
6. 已包含 `go.mod` 和空 `agent-studio.nodes.yaml` 的新项目 Module，可先执行 `node package init`，再执行 `node init`、`node test` 和 `generate` 完成开发闭环；许可证和仓库地址均来自显式输入。
