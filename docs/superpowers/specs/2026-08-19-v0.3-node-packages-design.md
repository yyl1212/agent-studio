...
| `registrations` | 是 | 至少一个、按 package 唯一 |
...
| B1 | 清单 Schema 与解析 | 2 天 | 全部格式、预算和规范化测试通过 |
...
总量预计 12–16 个净开发日，不包含外部评审等待。每阶段未通过自身退出条件时，不开始依赖它的下一阶段。
...
6. v1alpha1 模板可导入；v1alpha2 模板在相同环境下规范化字节往返稳定。
7. 缺失节点可展示包名和导出版本，但不能自动安装或绕过 Compiler。
8. API、CLI JSON、模板、日志公开字段中不出现本地绝对路径或 replace 目标。
9. Node Definition 的 capability、Schema 和端口仍来自实际注册节点，不被静态清单覆盖。
10. Go 全量、vet、Web 测试、Docker PostgreSQL 集成和 Playwright 全部通过。
11. 独立代码审查无 Critical 或 Important。
12. 实施分支从最新 `origin/main` 创建，不复用已合并的 v0.3-A 分支。
