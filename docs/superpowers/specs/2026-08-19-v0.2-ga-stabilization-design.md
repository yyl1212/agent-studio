...
1. 建立可由 PR 快照验证、由不可变标签触发的 CLI 制品流水线。
...
3. 只有四个平台全部成功后，最终任务才把同一份制品上传到不可见 Draft；远端复核通过后再公开。
...
- `checksums.txt` 使用 SHA-256，覆盖四个最终 `tar.gz` 归档。
- 每个归档生成一个 SPDX JSON SBOM，文件名包含对应归档名。
- SBOM 生成工具 Syft 与 GoReleaser 一样固定版本，不在标签工作流中浮动升级。
- 制品检查脚本验证归档数量、文件名、校验和、SBOM JSON 可解析性和目标映射。
...
`.github/workflows/release.yml` 只响应下列标签：
...
| `scripts/check-release-artifacts_test.sh` | 新增 | 制品脚本 fixture 测试 |
| `Makefile` | 修改 | 发布配置、snapshot 和制品验证目标 |
| `.gitignore` | 修改 | 忽略本地 `dist/` |
...
Linux arm64 Runner 当前由 GitHub 以 Public Preview 提供。D1 首项先验证本仓库能调度该 Runner；若不可用，必须先修订实施计划为 QEMU 执行方案并明确记录“模拟执行”，不得静默降低门禁。
...
审查结论：方案没有 Critical 或 Important 级阻塞项。D1 的首个实施任务必须验证 Runner 标签与 GoReleaser/Syft 固定版本的真实快照链路；该探针通过前不得继续编写发布权限和标签逻辑。
