...
| `docs/superpowers/plans/2026-08-19-v0.2-stage-c-rc-release.md` | 已新增 | 固化逐步实施、验证和远程确认点 |
...
标签推送后，在新的临时目录通过匿名公共 Go Proxy 和公共校验和数据库执行；必须清空本机私有模块例外，禁止 `direct` 回退和本机 Git 凭据参与：

```bash
rc_install_root="$(mktemp -d)"
mkdir -p "$rc_install_root/bin" "$rc_install_root/modcache" "$rc_install_root/buildcache"
CGO_ENABLED=0 GOPRIVATE= GONOPROXY= GONOSUMDB= GOPROXY=https://proxy.golang.org GOSUMDB=sum.golang.org GOBIN="$rc_install_root/bin" GOMODCACHE="$rc_install_root/modcache" GOCACHE="$rc_install_root/buildcache" go install github.com/yyl1212/agent-studio/cmd/agent-studio@v0.2.0-rc.1
"$rc_install_root/bin/agent-studio" version
```

允许为公共代理传播设置总时长不超过 60 秒的有限重试。输出必须包含构建版本 `v0.2.0-rc.1`、SDK `0.2.0` 和 API `agent-studio.dev/v1alpha1`。临时缓存和 `GOBIN` 避免复用本机模块缓存或覆盖用户现有 CLI。
