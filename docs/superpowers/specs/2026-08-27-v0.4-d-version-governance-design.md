...
| 引用组合、游标或请求 JSON 非法 | 400 | `INVALID_REQUEST` | 显示安全通用消息 |
...
| `apps/api/internal/workflow/version_governance.go` | 新增 | 列表、比较、回滚和撤销应用服务 |
| `apps/api/internal/workflow/version_diff.go` | 新增 | 纯语义差异引擎 |
| `apps/api/internal/workflow/ports.go` | 修改 | 版本治理 Store 端口 |
...
| 文件 | 动作 | 职责 |
|---|---|---|
| `apps/web/src/lib/api/generated.ts` | 生成 | OpenAPI 类型 |
...
包含迁移、领域类型、共享秘密识别、差异引擎、Store、应用服务、HTTP API、OpenAPI 和后端中文文档。该 PR 合并前不加入 Studio 入口。
