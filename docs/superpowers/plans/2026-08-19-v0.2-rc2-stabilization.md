...
`v0.2.0-rc.2` 是稳定化候选版，不增加产品功能。它延续 `rc.1` 的 SDK、节点协议、画布、运行时和官方节点行为，并首次通过 GitHub Release 提供 macOS/Linux 的 amd64、arm64 CLI 归档、SHA-256 校验和和逐归档 SPDX JSON SBOM。

## 本次变化

- 增加 Linux amd64、Linux arm64、macOS Intel 和 macOS Apple Silicon 四种预编译 CLI 归档。
- 每个归档均包含 `agent-studio`、`README.md` 和 `LICENSE`。
- 提供覆盖四个归档和四份 SBOM 的 `checksums.txt`。
- 为每个归档提供一份 SPDX 2.3 JSON SBOM。
- 四个平台在 Release 公开前分别原生解压并执行同一制品集合中的 CLI。
...
创建标签前必须通过快速验证、完整 Go/Web/PostgreSQL 回归、产品 E2E、SDK E2E、发布制品快照和独立审查。标签工作流还会验证 `main` HEAD、注释标签、四个平台原生执行、远端附件字节和 Pre-release 状态；任一门禁失败都不会公开 Release。
````
...
rg -n '`v0\.2\.0-rc\.2` 提供 Linux/macOS 的 amd64、arm64 CLI 归档' README.md
rg -n 'macOS 归档目前未签名，也未经过 Apple Developer ID 签名或公证' README.md
! rg -n '当前候选版本为 `v0\.2\.0-rc\.1`|`v0\.2\.0-rc\.1` 仍是只包含源码的版本' README.md
git diff --check
```

Expected: 五项 `rc.2` 与安全边界断言匹配，旧“当前版本”断言无匹配，差异检查通过。历史文档和兼容性示例中的 `rc.1` 不删除。
...
```bash
proxy_root=$(mktemp -d)
trap 'rm -rf -- "$rc2_dist" "$proxy_root"' EXIT HUP INT TERM
