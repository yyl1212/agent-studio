...
Expected: 六项 `rc.2`、远端生命周期条件与安全边界断言匹配，旧“当前版本”断言无匹配，差异检查通过。历史文档和兼容性示例中的 `rc.1` 不删除。
...
```bash
git diff --check
rg -n 'Webhook 与 Retriever 各 50 次重复测试' docs/releases/v0.2.0-rc.2.md
rg -n '标签工作流负责验证.*只有全部门禁成功才会公开 Release' docs/releases/v0.2.0-rc.2.md
```
