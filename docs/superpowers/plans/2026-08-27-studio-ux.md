...
- [ ] **Step 4: 更新工作台非模态语义并消除重复 Escape 监听**

删除 `WorkbenchPanel` 自己的 `window.keydown` Escape effect，最上层关闭统一由 `StudioShell` 负责，避免一次按键触发两个关闭处理器。在 `WorkbenchPanel.test.tsx` 断言 `role="dialog"`、标题关联、宽度夹紧、关闭按钮和 resizer 键盘行为仍然可用；不要添加 `aria-modal="true"` 或焦点圈定。
