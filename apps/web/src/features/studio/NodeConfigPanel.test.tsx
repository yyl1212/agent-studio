...
it('展示端口变化并用快捷键应用已就绪草稿', async () => {
  const onApply = vi.fn()
  const draft: UseNodeConfigDraftResult = { nodeId: 'node-a', draft: { mode: 'new' }, normalized: { mode: 'new' }, dirty: true, status: 'ready', preview: { ports: { inputs: [], outputs: [] }, added: ['output:new'], removed: ['output:old'], invalidEdgeIds: ['edge-1'] }, error: '', setDraft: vi.fn(), reset: vi.fn(), markApplied: vi.fn() }
  render(<NodeConfigPanel titleId="config-title" node={node} draft={draft} onApply={onApply} />)
