...
describe('RunDetailPanel', () => {
	...
	it('切换运行会取消旧详情，并安全渲染输出和终态调试链接', async () => {
		...
		await act(async () => second.resolve({ run: { id: 'two', workflowId: summary('two').workflowId, mode: 'test', status: 'failed', input: {}, output: '<script>bad</script>', startedAt: '2026-08-26T00:00:00Z' }, nodeRuns: [] }))
	...
	it('运行中的详情不显示调试回放', async () => {
		vi.spyOn(api, 'getRun').mockResolvedValue({ run: { id: 'one', workflowId: summary('one').workflowId, mode: 'test', status: 'running', input: {}, startedAt: '2026-08-26T00:00:00Z' }, nodeRuns: [] })
