...
export function RunManagementView() {
  ...
  return <section aria-labelledby="run-management-title">
    ...
    <div className="management-filters run-filters">
      ...
      <fieldset><legend>状态</legend>{(['queued', 'running', 'recovery_required', 'cancelling', 'completed', 'failed', 'cancelled'] as const).map((status) => <label key={status}><input type="checkbox" checked={parsed.query.statuses.includes(status)} onChange={() => toggleStatus(status)} />{statusLabel(status)}</label>)}</fieldset>
